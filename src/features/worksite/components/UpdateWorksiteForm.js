...
import useUpdateWorksiteMutation from "../hooks/useUpdateWorksiteMutation";
...
function UpdateWorksiteForm({ worksite, setIsUpdateMode }) {
    const {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await updateWorksite.mutateAsync(values);
                console.log("Chantier modifié avec succès :", response);
                setIsUpdateMode(false);
            } catch (error) {
                console.error(
                    "Erreur lors de la modifcation du chantier :",
                    error,
                );
                setError(error);
        ...
        >
            ...
            <form onSubmit={formik.handleSubmit} style={{ width: "80%" }}>
                <Typography
                    ...
                    textAlign="center"
                >
                    Modifier chantier : {worksite.name} - {worksite.city}
                </Typography>
...
export default UpdateWorksiteForm;
